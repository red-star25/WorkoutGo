...
class Popular_onClicked extends StatefulWidget {
  Map exercicseData;
  int index;
  Popular_onClicked({this.exercicseData,this.index});

  @override
  _Popular_onClickedState createState() => _Popular_onClickedState();
}

class _Popular_onClickedState extends State<Popular_onClicked> {

  Map steps;
  Future<void> getData()async{
    final databaseReference = Firestore.instance;
    await databaseReference.collection('popular_exercise').document("pop_${widget.index}").collection('steps').getDocuments().then((value) => value.documents.forEach((element) {steps=element.data;}));
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: <Widget>[
                Card(
                  ...
                  child: ClipRRect(
                    ...
                    child: CachedNetworkImage(
                      imageUrl: widget.exercicseData["image_url"],
                ...
                Center(
                  child:Text(
                    "${widget.exercicseData["exer_name"]}",
                ...
                Padding(
                  ...
                  child: RichText(
                    text: TextSpan(
                      children: [
                        for(int i =0;i<steps.keys.toList().length;i++)
                          TextSpan(
                              text: "Step:${i+1}\n${steps["step${i+1}"]}\n\n",
                              style: GoogleFonts.ubuntu(fontSize: 15,color: Colors.white,fontWeight: FontWeight.bold)
                          ),
                      ],
...
class Equipment extends StatelessWidget {
  ...
  final Popular_onClicked widget;
